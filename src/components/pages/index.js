...
export default class Pages extends PureComponent {
  ...
  onScroll(event) {
    ...
    let {
      [horizontal ? 'width' : 'height']: base, progress
    } = this.state;
  ...
  renderPager(pager) {
    ...
    return ( <
      View style = {
        [styles[indicatorPosition], indicatorStyle]
      } >
      <
      Indicator { ...pager
      }
  ...
  renderPages(props) {
    ...
    let {
      [horizontal ? 'width' : 'height']: base, layout
    } = this.state;
    ...
    return ( <
      ...
      style = {
        [styles.container, style, scrollStyle]
      }
      onScroll = {
        this.onScroll
      }
